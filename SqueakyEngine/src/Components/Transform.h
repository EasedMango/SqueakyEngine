...
public:
	...
	inline glm::quat GetRotation() const { return glm::quat(rotation); }
	inline glm::vec3 GetRotationEuler() const {
		return rotation;
	}
	inline void const SetPosition(const glm::vec3& pos_)  {  pos = pos_; }
	inline void const SetScale(const glm::vec3& scale_)  {  scale=scale_; }
	inline void const SetRotation(const glm::quat &rotation_)  { rotation = glm::eulerAngles(rotation_); }
